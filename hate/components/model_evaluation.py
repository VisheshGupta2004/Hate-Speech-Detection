...
class ModelEvaluation:
    ...
    def get_best_model_from_local(self) -> str:
        """
        :return: Fetch best model from local storage and store inside best model directory path
        """
        try:
            ...
            self.local_data_manager.sync_folder_from_local(self.model_evaluation_config.MODEL_NAME,
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifacts:
        ...
        try:
            ...
            logging.info("Check is best model present in the local storage or not ?")
            if os.path.isfile(best_model_path) is False:
                is_model_accepted = True
                logging.info("Best model not found in local storage; accepting the currently trained model as the best model.")
